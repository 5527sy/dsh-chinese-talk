 ...
 * `onUtterance`.
 *
 * Continuous listening: the CALLER keeps the recorder running after each
 * utterance (do not stop it in onUtterance) — the sidebar does exactly that,
 * so every sentence is auto-sent until the user clicks the mic again.
 */
...
export interface MicRecorderOptions {
  minSilenceMs?: number
  /** Push-to-talk: accumulate from press; the whole hold flushes once on stop. */
  ptt?: boolean
   ...
   *  audio below this level is faded out of the SENT mic stream. */
  noiseGateDb?: number
  ...
  /** Called when the "currently capturing speech" flag flips. */
...
export class MicRecorder {
  ...
  async start(): Promise<void> {
    ...
    // Arm the noise gate: quiet ambient audio below `noiseGateDb` is faded out
    // of the SENT stream. The worklet already carries the full
    // attack/hold/release envelope — this message enables it.
  ...
  stop(): void {
    if (this.released) return
    // PTT / mid-speech stop: flush whatever was captured during the hold as
    // the final utterance (silence endpointing would have flushed it later).
    if (this.opts.ptt === true || this.speaking) {
  ...
  private onLevel(rms: number): void {
    ...
    // PTT: every held moment counts as speech (no threshold gating).
    if (this.opts.ptt === true && !this.interruptMode) {
      if (!this.speaking) {
        this.speaking = true
        this.armMaxTimer()
  ...
  private onChunk(buffer: ArrayBuffer): void {
    ...
    if (this.opts.ptt !== true && silenceMs >= (this.opts.minSilenceMs ?? DEFAULT_MIN_SILENCE_MS)) this.flush()
  }
