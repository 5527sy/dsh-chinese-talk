...
/** Bridge health probe: { status, stt, tts, ... } or null when unreachable. */
export async function health(): Promise<{
  ...
  try {
    const resp = await fetch(`${bridgeBase()}/api/health`)
    if (!resp.ok) return null
    return resp.json() as Promise<{ status: string; stt?: boolean; tts?: boolean; voice?: string; voices?: string[] }>
  } catch {
    return null
  }
}
...
/** Voice (persona) list: [{ name, label, current }] + current. */
export async function voiceList(): Promise<{ voices: { name: string; label: string; current: boolean }[]; current: { voice: string } } | null> {
  try {
    const resp = await fetch(`${bridgeBase()}/api/persona/list`)
    if (!resp.ok) return null
    return resp.json() as Promise<{ voices: { name: string; label: string; current: boolean }[]; current: { voice: string } }>
  } catch {
    return null
  }
}

/** Hot-switch the TTS voice on the bridge. */
export async function setVoice(name: string): Promise<boolean> {
  try {
    const resp = await fetch(`${bridgeBase()}/api/persona/set`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
/**
 * Streaming silero VAD client for barge-in detection. While a reply is
 * playing the mic recorder pushes PCM16 chunks here; the bridge replies
 * `{ event: 'speech_start' }` only when a REAL human voice is detected —
 * TTS echo / music / ambient noise never trip it.
 */
